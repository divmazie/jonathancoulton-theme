...
$from_transient = get_transient('twitter_context');
if ($from_transient) {
...
foreach ($response as $raw_tweet) {
    ...
    $tweet = array('text'=>$text, 'ago'=>$ago, 'url'=>$url, 'raw'=>$raw_tweet);
    $tweets[] = $tweet;
}
$twitter = array('handle'=>$user, 'url'=>"http://twitter.com/$user",'tweets'=>$tweets);
set_transient('twitter_context',$twitter,60);

return array_merge($twitter,array('from'=>'api'));